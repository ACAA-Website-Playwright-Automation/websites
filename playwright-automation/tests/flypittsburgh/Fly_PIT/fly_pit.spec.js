...
test.describe('PIT test', () => {
  test('should match snapshot for all menu and submenu URLs', async ({ page }) => {
    ...
    const urlsToSkip = [
      ...
      // 'https://flypittsburgh.com/pittsburgh-international-airport/parking/parking-reservations/',
      // 'https://flypittsburgh.com/pittsburgh-international-airport/parking/parking-options/',
      // 'https://flypittsburgh.com/pittsburgh-international-airport/terminal-information/safety-security/',
      'https://map.flypittsburgh.com/',
      'https://neighborhood91.com/',
    ...
    try {
      ...
      //hide the gif
      await page.evaluate(() => {
        const gif = document.querySelector('.align-none > img');
        if (gif) {
      ...
      //pause the video
      const videoSelector1 = '.mejs-mediaelement video';

      const videoExists1 = await page.$$(videoSelector1); 
  
      if (videoExists1.length > 0) {
